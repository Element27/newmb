...
function readFallbackPlans(): WeeklyPlan[] {
  const file = getFallbackFile();
  if (!fs.existsSync(file)) return [];
  try {
    return JSON.parse(fs.readFileSync(file, "utf-8")) as WeeklyPlan[];
  } catch {
    return [];
  }
}

function writeFallbackPlans(plans: WeeklyPlan[]) {
  const file = getFallbackFile();
...
function resolveWeekStart(input?: string) {
  if (input && /^\d{4}-\d{2}-\d{2}$/.test(input)) return input;
  return toISODate(getNextMonday(new Date()));
}

function mapItems(data: any[]): WardrobeItem[] {
...
async function loadWardrobeItems(userId: string): Promise<WardrobeItem[]> {
...
async function loadUserPrimaryStyle(userId: string): Promise<string | null> {
  const supabase = getSupabaseServer();
  if (supabase) {
    const { data, error } = await supabase
      .from("profiles")
      ...
      .eq("user_id", userId)
      .maybeSingle();
    if (!error && data) return data.primary_style ?? null;
  }
  return null;
}

async function loadPlan(userId: string, weekStart: string): Promise<WeeklyPlan | null> {
  const supabase = getSupabaseServer();
  ...
  const fallback = readFallbackPlans().find(
    (plan) => plan.userId === userId && plan.weekStart === weekStart
  );
...
async function savePlan(plan: WeeklyPlan) {
  ...
  const existing = all.find(
    (item) => item.userId === plan.userId && item.weekStart === plan.weekStart
  );
  if (existing) {
...
export const getWeeklyPlan = async (req: Request, res: Response) => {
  ...
  const weekStart = resolveWeekStart(req.query.weekStart as string | undefined);

  const plan = await loadPlan(userId, weekStart);
  return res.json({ ok: true, weekStart, plan });
};

export const generateWeeklyPlanHandler = async (req: Request, res: Response) => {
  const authUser = await requireAuthenticatedUser(req, res);
  if (!authUser) return;
  const userId = authUser.id;
  const weekStart = resolveWeekStart(req.body.weekStart as string | undefined);

  const plan = await generatePlanForUser(userId, weekStart);
  return res.json({ ok: true, plan });
};

export const updateWeeklyPlanDay = async (req: Request, res: Response) => {
  const authUser = await requireAuthenticatedUser(req, res);
  if (!authUser) return;
  const userId = authUser.id;
  const weekStart = resolveWeekStart(req.body.weekStart as string | undefined);
  ...
  const patch: Partial<WeeklyPlanDay> = {};
  if (Array.isArray(req.body.itemIds)) {
    patch.itemIds = req.body.itemIds.map((item: unknown) => String(item));
  }
...
export const regenerateWeeklyPlanDay = async (req: Request, res: Response) => {
  const authUser = await requireAuthenticatedUser(req, res);
  if (!authUser) return;
  const userId = authUser.id;
  const weekStart = resolveWeekStart(req.body.weekStart as string | undefined);
  ...
  const candidates = items.filter(
    (item) => !basePlan.days.some((day) => day.itemIds.includes(item.id) && day.date !== date)
  );
