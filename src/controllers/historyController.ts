 ...
 */
export const logOutfitHistory = async (req: Request, res: Response) => {
  const authUser = await requireAuthenticatedUser(req, res);
  if (!authUser) return;
  const userId = authUser.id;

  const { itemIds, occasion, wornAt, type = "worn" } = req.body as {
    itemIds: string[];
    occasion?: string;
    wornAt?: string;
    type?: "worn" | "suggested";
 ...
 */
export const getOutfitHistory = async (req: Request, res: Response) => {
  const authUser = await requireAuthenticatedUser(req, res);
  if (!authUser) return;
  const userId = authUser.id;
  ...
  const map = new Map<string, { wornCount: number; suggestedCount: number; lastWornAt: string | null }>();
