...
export function recommendOutfit(items: WardrobeItem[], occasion: "work" | "casual" | "travel"): WardrobeItem[] {
  const byCat = new Map<WardrobeItem["category"], WardrobeItem[]>([
    ["top", []],
    ["bottom", []],
    ["shoes", []],
    ["outer", []],
  ...
  const pick = (list: WardrobeItem[], preferNeutral = false): WardrobeItem | null => {
  ...
  if (dress && shoes) {
    ...
    if (occasion !== "casual" && outer) result.push(outer);
  } else {
    if (top) result.push(top);
    if (bottom) result.push(bottom);
    if (shoes) result.push(shoes);
    if (occasion === "travel" && outer) result.push(outer);
  }

  if (occasion === "casual" && accessory) result.push(accessory);
