...
Respond ONLY with the JSON object, no markdown formatting.`;
  ...
  let finalCategory = manualCategory as Category || null;
  
  if (!manualCategory && parsed.category && validCategories.includes(parsed.category.toLowerCase())) {
      finalCategory = parsed.category.toLowerCase() as Category;
  }
...
export async function classifyImage(
  ...
  try {
    if (process.env.GEMINI_API_KEY) {
      ...
      return { label: "unknown", category: manualCategory as Category || null, confidence: 0 };
    }
  } catch (error) {
...
export async function extractAverageColorHex(imagePath: string): Promise<string> {
  try {
